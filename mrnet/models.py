...
class Model(object):
    ...
    def save(self, sess=None,epoch=''):
        if not sess:
            raise AttributeError("TensorFlow session not provided.")
        saver = tf.train.Saver(self.vars)
        save_path = saver.save(sess, "Data/checkpoint"+str(epoch)+"/%s.ckpt" % self.name)
        print("Model saved in file: %s" % save_path)
    ...
    def load(self, sess=None):
        if not sess:
            raise AttributeError("TensorFlow session not provided.")
        saver = tf.train.Saver(self.vars)
        save_path = "Data/checkpoint10/%s.ckpt" % self.name #checkpoint13_rotate
        saver.restore(sess, save_path)
        print("Model restored from file: %s" % save_path)

    def load_test(self, sess=None):
        if not sess:
            raise AttributeError("TensorFlow session not provided.")
        saver = tf.train.Saver(self.vars)
