...
# Modifications: revise the mesh_loss() and add a pc_loss()
...
def mesh_loss(pred, placeholders, block_id):
	...
	point_loss = 0.5*(tf.reduce_mean(dist1) + tf.reduce_mean(dist2))#* 3000

	pc_loss = tf.reduce_mean(tf.abs(gt_pt-pred))*1000
	...
	total_loss = point_loss + edge_loss + normal_loss + pc_loss
	return total_loss

def pc_loss(pred, placeholders):
	gt_pt = placeholders['labels'][:, :3] # gt points
	point_loss = tf.reduce_mean(tf.abs(gt_pt-pred))
	return point_loss*10
